...
<div class="breadcrumbs">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                                        ...
                                        <tbody>
                                        @foreach ($doneproyeks as $done)
                                            <tr>
                                              <td>{{ $loop->iteration }}</td>
                                              <td>{{ $done->proyek->no_tiket_proyek }}</td>
                                              <td>{{ $done->proyek->user->employee->name }}</td>
                                              <td>{{ $done->proyek->donesale->sale->client->company_name }}</td>
        ...
        <div class="modal-body">
            <form action="/dashboard/proyeks/printLaporan" method="post" class="d-inline" target="_blank">
                      @method('get')
                      @csrf
                      <label for="tanggal_awal" class="form-label">Tanggal Awal</label>
                        ...
                        @enderror
                                ...
                                <form action="/dashboard/proyeks/printLaporan"  target="_blank" method="post" class="d-inline mb-3 mr-3">
                                        @method ('get')
                                            @csrf
